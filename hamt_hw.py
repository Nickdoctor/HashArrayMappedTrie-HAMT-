...
# Written by Nicolas Gugliemo, ngugliemo@csus.edu, 2/19/2024, Fall 2024
# 
...
class hamt:
    ...
    def _contains(self, item, hashbits):
        if self._item == item:
            # This node matches item. Return True
            return True
        else:
            # Continue search using hashbits to pick direction
            child_num = hashbits % hamt.DEG
            if self._children[child_num] == None:
                # item not in trie. Return False
                return False
            else:
                shiftedhashbits = hashbits // hamt.DEG
...
class HamtIter:
    ...
    def __init__(self, node):
        self._node = list(filter(None, node._children))
    ...
    def __next__(self):
        if not self._node:
            ...
            node = self._node.pop()
            self._node.extend(filter(None, node._children))
            return node._item
...
if __name__ == '__main__':
    ...
    print("NEXT TEST, SHOULD PRINT 2 FALSE")
    print(f.contains("A"))
    print(f.contains("B"))
    print(f.contains("C"))
    print(f.contains("D"))
    ...
    print("NEXT TEST, SHOULD BE 5 LENGTH")
    for x in f:
        print(x)
    print("NEXT TEST, SHOULD BE 4 LENGTH")
    for y in e:
        print(y)
